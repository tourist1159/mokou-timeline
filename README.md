...
- 統合タイムライン
- **並び順**: 新しい順／古い順／再生数順／コメントの多い順／コメントが速い順。
  「速い順」は**1時間あたりのコメント数**（コメント数 ÷ 長さ）で並べる。長さが分からないもの
  （配信中のカードなど）は出せないので末尾へ送る。この順を選ぶと、カードの表示も自動で
  速さ側に切り替える（並びの理由が見えないため。コメント数をクリックすれば総数に戻せる）。
  再生数・コメント数は「多い順」で、
...
- **検索（タイトル＋タグ）**: 検索ボックスの入力はタイトルとタグの両方に当たる。
  タイトルに出てこない語でもタグ経由で拾える（例:「加藤純一」「切り抜き」はタイトルには
  1本も無いが、タグとしては13本に付いている）。
- **タグ候補のサジェスト**: 文字を入力すると、その文字列を含むタグが**件数付き**で候補に出る。
  選ぶとそのタグを持つ動画だけに絞り込み、解除できるチップ（🏷）が上部に出る。`?tag=…` で共有可能。
  - 候補の並びは**前方一致を優先 → 件数の多い順**、最大10件
  - 件数を併記しているのは、`もこう`(53/53本) `ゲーム`(49/53本) のように**ほぼ全動画に付いていて
    絞り込みに使えないタグ**があるため。隠さずに件数で判断できるようにしている
  - 照合は `normalizeText()`（小文字化・NFKC・空白と!?の除去）なので、`マリオカート ワールド` は
    空白を抜いて打っても引ける。`Pokemon Champions` と `PokemonChampions` のような表記ゆれも
    同じタグとして統合される
  - タグは YouTube のみ（`youtube_archives.json` の `tags`）。fetcher が12時間ごとに取り直す
- 件数・期間の統計表示
- カード表示（サムネ・長さ・日時(JST)・バッジ・再生数・コメント数・元動画へのリンク）
- **コメント数の表示**: 再生数と同じく概数（`61,950` → `💬 6.2万`）。**クリック/タップで
  「1時間あたりのコメント数（流れの速さ）」に切り替わる**（`⚡ 8,082/時`）。切り替えは全カード
  まとめて効き、`localStorage` に保存される。マウスオーバーで内訳（全件数と長さ）が出る。
  カード自体がリンクなので、押しても動画へ飛ばないよう `preventDefault`（グラフボタンと同じ作り）
...
- フィルタ/検索/並び/期間/タグは URL クエリに反映（共有可能。`?from=2026-08-01&to=2026-08-31` など。
