...
- **メンバー限定**: YouTube のメンバー限定配信（`members_only`）はカードに「メンバー限定」タグを出す。
  YouTube 側が再生数を公開していないので、これらに 👁 は付かない
- **再生数**: YouTube のみ（`youtube_archives.json` の `view_count`）。`👁 12.3万` のように概数で出し、
