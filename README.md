...
- 統合タイムライン
- **並び順**: 新しい順／古い順／再生数順／コメントの多い順。再生数・コメント数は「多い順」で、
  値を持たないもの（再生数は YouTube 以外、コメント数は未取得のもの）は末尾へ送る
  （0扱いにすると「0回」と「未取得」が混ざるため）。日付順以外では**時間軸の日付見出しと
  新着境界線を自動的に切る**（どちらも「並びが日付順」であることが前提の表示なので、
  再生数順だと1件ずつ見出しが並んでしまう）。設定自体は保持され、日付順に戻すと復帰する
- フィルタ: 配信元（YouTube / Kick）・チャンネル・種別（配信 / 動画）
- **期間フィルタ**: 開始日〜終了日を指定してその範囲の動画・配信だけを表示する（両端を含む）。
  デスクトップは **📅ボタンを押すと出るポップオーバー**、モバイルは「絞り込み」パネルの中
  （パネル自体が開閉式なので二重には畳まない）。期間が効いているときは📅ボタンの枠色が変わる。
  日付の比較は **JST 基準の日付キー（`YYYY-MM-DD`）同士の文字列比較**で行う。ゼロ埋め固定長
  なので辞書順＝日付順になり、タイムゾーン計算をせずに端の日を「その日いっぱい」含められる
  （タイムライン側の日付グループとも同じ基準）。逆順に指定した場合は自動で入れ替える。
  片方だけの指定（それ以降／それ以前）も可
- タイトル検索
- 件数・期間の統計表示
...
- フィルタ/検索/並び/期間は URL クエリに反映（共有可能。`?from=2026-08-01&to=2026-08-31` など。
  日付として成立しない値は無視してフィルタを効かせない）
- **コメント流量グラフ**: コメントデータがある配信（カードに📊ボタン）はクリックでモーダル表示。
  ...
  - **キーワードは編集できる**: モーダル内の「🔧 キーワード集計」を開くと、正規表現を1件
    ずつ入力欄で追加・削除・編集できる（既定は `草|w` / `8{3,}` / `^あ+$`）。変更は即座に
    グラフへ反映され、`localStorage` に保存して次回以降も使う。1本の入力欄1パターンなのは、
    `8{3,}` のようにパターン自体にカンマを含むものがあり、カンマ区切りの単一文字列では
    表現できないため。
...
- **モバイルの絞り込み**: 配信元・種別・チャンネル・期間は「表示設定」と同じ行にある
...
- **モバイルのコントロール行**: 「並び」（ラベルなし・selectのみ）と検索バーを1行に、
  「表示設定」と「絞り込み」を1行にまとめる（`.control-row`。デスクトップでは
  `display: contents` にして中身をそのまま `.controls` の並びに乗せるだけの箱で、
  見た目は変わらない）。並びの select は内容ぶんの幅のまま伸縮させず、余った幅は
  検索バー側に回す（コメントの多い順のような長いラベルを選んでも、検索バーが縮んで
  1行に収まる。320px 幅でも横スクロールは出ない）。
